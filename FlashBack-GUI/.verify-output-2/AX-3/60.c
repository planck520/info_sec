 ...
 *    - formSetIPv6status (arg=1, call_ea=0x64f3c, func_ea=0x62228, label=needs_check)
 *        call: GetValue("ipv6.wan.pppoe.username", v207);
 *    - setPppoeConfig (arg=1, call_ea=0x2ec34, func_ea=0x2e900, label=certain)
 *        call: SetValue("ipv6.wan.pppoe.username", Var);
 *    - websGetVar (arg=ret, call_ea=0x2eb88, func_ea=source, label=source)
 *        call: Var = websGetVar(wp, "adslUser2", &byte_794DF);
 * ===================================================== */
...
/* Function: setPppoeConfig @ 0x2E900 */
void __fastcall setPppoeConfig(webs_t wp, int wan_id)
{
  char_t *Var; // r8
  char_t *v5; // r7
  char *v6; // r11
  char *v7; // r9
  char *v8; // r4
  char_t mib_name[256]; // [sp+8h] [bp-328h] BYREF
  char_t pppoename[256]; // [sp+108h] [bp-228h] BYREF
  char_t pppoepwd[256]; // [sp+208h] [bp-128h] BYREF

  memset(mib_name, 0, sizeof(mib_name));
  memset(pppoename, 0, sizeof(pppoename));
  memset(pppoepwd, 0, sizeof(pppoepwd));
  if ( wan_id == 1 )
  {
    Var = websGetVar(wp, "adslUser", &byte_794DF);
    v5 = websGetVar(wp, "adslPwd", &byte_794DF);
    v6 = websGetVar(wp, "dnsAuto", "1");
    v7 = websGetVar(wp, "dns1", &byte_794DF);
    v8 = websGetVar(wp, "dns2", &byte_794DF);
    memset(mib_name, 0, sizeof(mib_name));
    sprintf(mib_name, "wan%d.ppoe.userid", 1);
    GetValue(mib_name, pppoename);
    memset(mib_name, 0, sizeof(mib_name));
    sprintf(mib_name, "wan%d.ppoe.pwd", 1);
    GetValue(mib_name, pppoepwd);
    if ( strncmp(pppoename, Var, 0x100u) || strncmp(pppoepwd, v5, 0x100u) )
    {
      setWanValue(1, "pppoe.auth.changed", "1");
      setWanValue(1, "ppoe.sev", &byte_794DF);
      setWanValue(1, "ppoe.ac", &byte_794DF);
    }
    SetValue("wl.wisp.access_mode", "pppoe");
    SetValue("wl.wisp.ip", &byte_794DF);
    SetValue("wl.wisp.mask", &byte_794DF);
  ...
  {
    Var = websGetVar(wp, "adslUser2", &byte_794DF);
    v5 = websGetVar(wp, "adslPwd2", &byte_794DF);
    v6 = websGetVar(wp, "dnsAuto2", "1");
  ...
  setWanValue(wan_id, "ppoe.userid", Var);
  setWanValue(wan_id, "ppoe.pwd", v5);
  SetValue("ipv6.wan.pppoe.username", Var);
  SetValue("ipv6.wan.pppoe.password", v5);
  setWanValue(wan_id, "pppoe.dns.auto", v6);
  if ( strlen(v7) > 6 || strlen(v8) > 6 )
  {
    setWanValue(wan_id, "pppoe.dns.hand1", v7);
    setWanValue(wan_id, "pppoe.dns.hand2", v8);
  }
}
