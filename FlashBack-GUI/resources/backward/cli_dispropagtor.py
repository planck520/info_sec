# -*- coding: utf-8 -*-
"""CLI entry for backward taint tracing (Hex-Rays based) - DISABLE PROPAGATOR & FORMAT_FUNCS VERSION.

This variant is used for RQ2. It disables indirect propagation through
propagator functions and ignores format_funcs so that we can measure how much
these modeled behaviors contribute to the analysis.

Changes:
1. Do not load propagators or format_funcs from the config.
2. Pass empty dictionaries to Analyzer and trace_data_flow.
3. Mark the output clearly as running with propagators and format_funcs disabled.
"""
...
class Analyzer:
    ...
    Attributes:
        ...
        format_funcs: Forced to an empty dictionary in this variant.
        propagators: Forced to an empty dictionary in this variant.
    ...
    def __init__(self, cwe_sources: Dict[Union[int, str], Set[str]], format_funcs: dict, propagators: dict, max_cache_size: int, max_depth: int = 10) -> None:
        ...
        # Disable format_funcs by forcing an empty dictionary
        # Original code: format_funcs or {}
        # Disable propagators by forcing an empty dictionary
        # Original code: propagators or {}
    ...
    def process_single_sink(self, cwe_name: str, sink_func: str, param_idx: int, len_idx: Optional[int] = None) -> List[dict]:
        # Disable format_funcs and propagators by passing empty dictionaries
        results = trace_data_flow(
            ...
            format_funcs={},  # Disable format_funcs
            vuln_type=cwe_name,
            len_idx=len_idx,
            propagators={},  # Disable propagators
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Backward taint tracing (Hex-Rays) - DISABLE PROPAGATOR")
...
def _setup_logging(log_level: str, output_path: str, firmware_name: str) -> str:
    ...
    log_file = os.path.join(log_dir, f"analysis_noprop_{firmware_name}_{timestamp}.log")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    _LOGGER.info(f"=" * 60)
    _LOGGER.info(f"Start analyzing firmware (PROPAGATOR & FORMAT_FUNCS DISABLED): {firmware_name}")
    ...
    # Load from the config but ignore propagators
    ...
    # Disable propagators and format_funcs by passing empty dictionaries even if the config provides them
    _LOGGER.info("[*] PROPAGATOR & FORMAT_FUNCS DISABLED - this variant disables indirect propagators and format-function handling for RQ3 validation")
    analyzer = Analyzer(
        cwe_sources=cwe_sources,
        format_funcs={},  # Force an empty dictionary
        propagators={},  # Force an empty dictionary
    ...
    if "Tenda" in binary_path or "tenda" in binary_path:
        _LOGGER.info("[*] Detected Tenda firmware; enabling main-function filtering...")
        results = _filter_tenda_paths_without_main(results)

    _LOGGER.info("[*] CWE-120 safe patterns (strlen+malloc/len) were filtered automatically during analysis")
    ...
    _LOGGER.info("")
    _LOGGER.info("=" * 60)
    _LOGGER.info("=== Analysis Complete (PROPAGATOR & FORMAT_FUNCS DISABLED) ===")
    _LOGGER.info("=" * 60)
    ...
    _LOGGER.info("")
    _LOGGER.info("=== Propagator & Format_Funcs Status ===")
    _LOGGER.info("Propagator & Format_Funcs DISABLED - this variant disables indirect propagators and format-function handling for RQ3 validation")
    _LOGGER.info("")
