 ...
 *    - formSetIPv6status (arg=1, call_ea=0x65024, func_ea=0x62228, label=needs_check)
 *        call: GetValue("wan1.ppoe.userid", v205);
 *    - form_fast_setting_pppoe_set (arg=1, call_ea=0x2cfec, func_ea=0x2cf68, label=certain)
 *        call: SetValue("wan1.ppoe.userid", Var);
 *    - websGetVar (arg=ret, call_ea=0x2cfcc, func_ea=source, label=source)
 *        call: Var = websGetVar(wp, "username", &byte_794DF);
 * ===================================================== */
...
/* Function: form_fast_setting_pppoe_set @ 0x2CF68 */
void __fastcall form_fast_setting_pppoe_set(webs_t wp, char_t *path, char_t *query)
{
  char_t *Var; // r9
  char_t *v5; // r8
  int v6; // r0
  int v7; // r2
  char old_connectType[8]; // [sp+0h] [bp-8h] BYREF
  char ret_buf[68]; // [sp+8h] [bp+0h] BYREF

  memset(ret_buf, 0, 0x40u);
  memset(old_connectType, 0, sizeof(old_connectType));
  GetValue("wan1.connecttype", old_connectType);
  Var = websGetVar(wp, "username", &byte_794DF);
  v5 = websGetVar(wp, "password", &byte_794DF);
  SetValue("wan1.ppoe.userid", Var);
  SetValue("wan1.ppoe.pwd", v5);
  v6 = SetValue("wan1.connecttype", "2");
  if ( CommitCfm(v6) )
  {
    doSystemCmd("cfm post multiWAN CfgChange1");
