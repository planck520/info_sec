 ...
 *    - set_wl_guest_iplist (arg=5, call_ea=0x4c05c, func_ea=0x4bc48, label=needs_check)
 *        call: sprintf(qos_iprule, "%d;%s;%s;%s", 1, dev_id, up_speed, down_speed);
 *    - set_wl_guest_qos_list (arg=1, call_ea=0x4d3fc, func_ea=0x4d3e0, label=certain)
 *        call: set_wl_guest_iplist(shared_up_speed, shared_down_speed);
 *    - fromSetWifiGusetBasic (arg=1, call_ea=0x3f140, func_ea=0x3e9c8, label=certain)
 *        call: set_wl_guest_qos_list(shared_up_speed, Var);
 *    - websGetVar (arg=ret, call_ea=0x3eb0c, func_ea=source, label=source)
 *        call: Var = websGetVar(wp, "shareSpeed", "0");
 * ===================================================== */


/* Function: set_wl_guest_iplist @ 0x4BC48 */
void __fastcall set_wl_guest_iplist(char *up_speed, char *down_speed)
{
  int id_list; // r11
  int v3; // r6
  char wl_guest_startip[16]; // [sp+24h] [bp-384h] BYREF
  char wl_guest_endip[16]; // [sp+34h] [bp-374h] BYREF
  char s[16]; // [sp+44h] [bp-364h] BYREF
  char dev_id[32]; // [sp+54h] [bp-354h] BYREF
  char qos_iprule[128]; // [sp+74h] [bp-334h] BYREF
  int cgi_debug[35]; // [sp+F4h] [bp-2B4h] BYREF
  char mib_value[256]; // [sp+180h] [bp-228h] BYREF
  char buf[256]; // [sp+280h] [bp-128h] BYREF
  ...
  id_list = bm_get_id_list("bandwidth.mode.listnum", cgi_debug, 35);
  memset(s, 0, sizeof(s));
  if ( GetValue("cgi_debug", s) && !strcmp("on", s) )
    printf(
      "%s[%s:%s:%d] %sget %d qos rules\n\x1B[0m",
      "\x1B[0;33m",
      "cgi",
      "get_idle_rule_id",
      507,
  ...
  if ( id_list <= 0 )
  {
    memset(s, 0, sizeof(s));
    if ( GetValue("cgi_debug", s) && !strcmp("on", s) )
      printf("%s[%s:%s:%d] %sFirst rule of qos\n\x1B[0m", "\x1B[0;33m", "cgi", "get_idle_rule_id", 511, "\x1B[0;32m");
    v3 = 1;
  }
  else
  {
...
LABEL_16:
  ...
  if ( GetValue("cgi_debug", cgi_debug) && !strcmp("on", (const char *)cgi_debug) )
    printf(
  ...
  if ( v3 )
  {
    ...
    sprintf(mib_value, "bandwidth.mode.list%d", v3);
    ...
    if ( mib_value[0] )
    {
      memset(cgi_debug, 0, 0x10u);
      strcat(mib_value, ",");
      snprintf((char *)cgi_debug, 0x10u, "%d", v3);
      strcat(mib_value, (const char *)cgi_debug);
    }
    else
    {
      snprintf(mib_value, 0x100u, "%d", v3);
    }
    memset(cgi_debug, 0, 0x10u);
    if ( GetValue("cgi_debug", cgi_debug) )
    {
      if ( !strcmp("on", (const char *)cgi_debug) )
        printf(
          "%s[%s:%s:%d] %sset %s == %s\n\x1B[0m",
          "\x1B[0;33m",
          "cgi",
...
/* Function: set_wl_guest_qos_list @ 0x4D3E0 */
void __fastcall set_wl_guest_qos_list(char *shared_up_speed, char *shared_down_speed)
{
  unset_wl_guest_list();
