...
function checkData() {
	G.validate = $.validate({
		custom: function () {
			...
			if ($("#dhcpEn").val() == "1") {
				var startIpLast = parseInt($("#startIp").val(), 10);
				var endIpLast = parseInt($("#endIp").val(), 10);
				if (startIpLast >= endIpLast) {
					return _("The end IP address must be greater than the start IP address.");
				}

				var lanIpLast = parseInt(lanIp.split('.')[3], 10);
				if (lanIpLast < startIpLast || lanIpLast > endIpLast) {
					return _("The LAN IP Address must be in the range of IP Address.");
				}
