...
function jumpTo (address, callback) {
	...
	var checkDomainRebootT = setInterval(function () {
		$.ajax({
			type: "get",
			url: "https://" + localDomain + "/goform/getRebootStatus?random=" + Math.random(),
			dataType: "jsonp",
			...
			success: function (json) {
				...
				top.location.href = "https://" + localDomain;
	...
	setTimeout(function () {
		...
		if (localDomain != address) {
			checkRebootT = setInterval(function () {
				$.ajax({
					type: "get",
					url: "https://" + address + "/goform/getRebootStatus?random=" + Math.random(),
					dataType: "jsonp",
					jsonp: "callback", //传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名(一般默认为:callback)
					jsonpCallback: "flightHandler", //自定义的jsonp回调函数名称，默认为jQuery自动生成的随机函数名，也可以写"?"，jQuery会自动为你处理数据
					success: function (json) {
						...
						clearInterval(checkRebootT);
						clearInterval(checkDomainRebootT);
						top.location.href = "https://" + address;
					}
				});
			}, 2000);
		}
...
var tooltipBoxMsg = "";
function mouseoverDevice(event) {
	mouseoutDevice();
	var mac = $(this).attr("data-mac");
	var boxRect = this.getBoundingClientRect();
	var boxLeft = boxRect.right + 10 + document.documentElement.scrollLeft;
	var boxTop = boxRect.top + (boxRect.bottom - boxRect.top) / 2 - 26 + document.documentElement.scrollTop;
	if(!mac) {
		return;
	}
	var $elem  = $("<div class='tooltip-box' style='left: " + boxLeft + "px;top: "+ boxTop + "px'>MAC: " + mac + "</div>");
	$("body").append($elem);
	tooltipBoxMsg = $elem;
}

function mouseoutDevice() {
	$(tooltipBoxMsg).remove();
}
