...
function PageLogic(pageView, pageModel) {
    ...
    this.initModule = function (id) {
        ...
        G.initPage = true;
				// console.log($("#adv_iptv").attr("disabled",true));
...
staInfo = {
    ...
    init: function () {
        ...
        if (!staInfo.loading) {
            ...
            //本机mac
            $("#routerStatus").on("mouseover", mouseoverDevice);
            $("#routerStatus").on("mouseout", mouseoutDevice);

            //上级mac
            $("#apModeImg").on("mouseover", mouseoverDevice);
            $("#apModeImg").on("mouseout", mouseoutDevice);

            $("#meshList").on("mouseover", ".status-mesh-li", mouseoverDevice);
            $("#meshList").on("mouseout", ".status-mesh-li", mouseoutDevice);
            $(window).on("scroll", mouseoutDevice);
    ...
    setImage: function (obj) {
        ...
        $("#routerStatus").attr("data-mac", obj.deviceMac);
        ...
        if(obj.nodeType == "agent") {
            ...
            //更新上级节点名称
            $("#upperDeviceName").html(toHtmlCode(obj.upperDeviceName));
            $("#apModeImg").attr("data-mac", obj.upperDeviceMac)
            if (obj.upperLine == "0" || obj.upperLine == "1" || obj.upperLine == "2") {
                $("#statusLine").html("<img src='/img/link-type-"+ obj.upperLine +".png'>");
                //子节点时必定是连接状态
		...
		if(G.alertIndex < 2) {
			obj["zjdxReportStatus"] === "1" && (G.alertIndex+=1) && alert("本设备限在浙江电信网下使用， 请切换网络重试。");
			obj["zjdxReportStatus"] === "2" && (G.alertIndex+=1) && alert("设备状态异常，请拨打 10000 号反馈。");
		}
    },
    ...
    showWorkMode: function (workMode, nodeType) {
        ...
        if (workMode == "ap" || workMode == "client+ap") {
            ...
            hideArr = ["wrl_guest", "adv_netcontrol", "adv_firewall", "adv_ddns", "adv_macFilter", "adv_virtualServer", "adv_dmz", "adv_iptv", "adv_route", "adv_upnp", "ip_mac_bind", "sys_wan", "adv_remoteweb"];
            if (workMode == "client+ap") {
                hideArr.push("adv_sleepMode", "wrl_meshSwitch");
            }
        }
        if(nodeType === "agent") { //子节点信息
            ...
            //次节点下隐藏以下模块
            hideArr.push("wrl_ssid_pwd", "wrl_channel", "wrl_signal", "wrl_access", "wrl_guest", "wrl_wps", "wrl_ofdma", "wrl_ap_mode","wrl_bridge")
        }
    ...
    setMeshStatus: function(obj) {
        ...
        for(; i < len; i++) {
            ...
            str += "<div class='status-mesh-li'  data-mac='"+obj.meshList[i].mac+"'  data-ip='"+obj.meshList[i].ip+"'><div class='status-mesh-icon "+ deviceClass +"'></div><div>"+obj.meshList[i].deviceName+"</div><div class='status-mesh-type'><img src='/img/link-type-"+ obj.meshList[i].line +".png'></div></div>";
        }
        $("#meshList").html(str);
    }
};
...
wrlInfo = {
    ...
    getIframe: function () {
        ...
        switch (id) {
            ...
            case "wrl_wifi_time":
                ...
                break;
            case "wrl_bridge":
                if ($(this).hasClass("disabled-wisp")) {
                    alert("请先关闭“MESH组网扩展”功能，再开启无线中继！");
                    return;
                }
                showIframe(_("Wireless Repeating"), "wisp.html", 700, 350);
                break;
    ...
    setValue: function (obj) {
        ...
        //AP模式 或路由模式下 无线中继和mesh组网互斥
        if(top.G.workMode == "router" || (top.G.workMode == "ap" && top.G.nodeType == "controller")) {

            if(obj.meshEn == "1") {
                $("#wrl_bridge").addClass("disabled-wisp");
            } else {
                $("#wrl_bridge").removeClass("disabled-wisp");
            }
        }
    ...
    setValue: function (obj) {
        ...
        if (obj.wl_mode == "apclient") {
            ...
            $("#adv_netcontrol, #adv_parental, #adv_remoteweb, #adv_ddns, #adv_upnp, #adv_virtualServer, #adv_dmz").removeClass("disabled");
				...
				//if (obj.wl_mode == "wisp"){
					//	$('#adv_iptv').addClass('disabled');
				//}else{
				//		$('#adv_iptv').removeClass('disabled');
				//}
        if (obj.netControl == 0) {
        ...
        if (obj.elink === "0") {
            $("#adv_eLink .function-status").html(_("关闭"));
        } else {
            $("#adv_eLink .function-status").html(_("开启"));
        }
    ...
    getIframe: function () {
        ...
        switch (id) {
            ...
            case "adv_tr069":
                ...
                break;
            case "adv_iptv":
                if(window.CONFIG_TENDA_IMAGE_HEAD == "AX1806_V2") {
            ...
            case "adv_firewall":
                ...
                break;
            case "adv_eLink":
                showIframe(_("启用与智能网关的无线配置同步"), "eLink.html", 580, 510);
                break;
        }
        G.iframeFlag = true;
    }
