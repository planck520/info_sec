...
case "$1" in
	start)
		...
		if [ /dev/root -ef /dev/rootfs1 ] || [ /dev/root -ef /dev/rootfs2 ]; then
			...
			# check andlinkv4 green net netmonitor 
			if [ -L /dev/mtdblock8 ]; then
				echo ">>>>> Mounting eMMC mtd8 Partition <<<<<"
				mount -t ext4 /dev/mtdblock8 /var/tmp/cfg -rw;
				if [ ! $? -eq 0 ]; then
					echo ">>>>> Formatting eMMC mtd8 Partition <<<<<"
					mke2fs -t ext4 -F /dev/mtdblock8;
					mount -t ext4 /dev/mtdblock8 /var/tmp/cfg -rw;
				fi
			fi
		else
			...
			# if mounting misc2 partition as UBIFS fails then mount as JFFS2
			if TEMP=`grep METADATA /proc/mtd` && MTD=`grep misc2 /proc/mtd`; # if pure UBI image grab line with "misc2"
			then # if this is a pureUBI image attempt to create/mount data partition as UBI if none exists or already UBI
				MTD=${MTD/mtd/}; # replace "mtd" with nothing
				MTD=${MTD/:*/}; # replace ":*" (trailing) with nothing
				if UBI=`ubiattach -m "$MTD"`; 	# try to attach data partition mtd to ubi, will format automatically if empty, 
								...
								# (i.e. JFFS2 has previously mounted this partition and written something)
				then # ubiattach was successful, mount UBI
					echo ">>>>> Mounting misc2 partition as UBIFS <<<<<"
					UBI=${UBI##*"number "}; # cut all before "number ", still need to get rid of leading space
					UBI=${UBI%%,*}; # cut all after ","
					DATA_PNAME=`ubinfo /dev/ubi"$UBI" -a | grep -o misc2`;
					# if data partition already exists, do not invoke ubimkvol
					if [ "$DATA_PNAME" != "misc2" ]; then
						echo ">>>>> Creating ubi volume ubi$UBI:misc2 <<<<<"
						ubimkvol /dev/ubi"$UBI" -m -N misc2; 
					fi
					mount -t ubifs ubi"$UBI":misc2 /var/tmp/cfg;
				else # otherwise mount as JFFS2
					echo ">>>>> Mounting misc2 partition as JFFS2 <<<<<"
					mount -t jffs2 mtd:misc2 /var/tmp/cfg -rw;
				fi
			else # otherwise if this is not a pureUBI image mount data partition as JFFS2 to allow for backwards compatibility
				echo ">>>>> Mounting misc2 partition as JFFS2 <<<<<"
				mount -t jffs2 mtd:misc2 /var/tmp/cfg -rw;
		...
		# Copy the green Internet plug-in
		# cp -rf /etc/green_plugin/* /tmp/netmonitor/
		# chmod 777 /tmp/netmonitor/* -R

		mknod /var/fuse c 10 229
		chmod a+rw /var/fuse
		mkdir -p /var/log /var/run /var/state/dhcp /var/ppp /var/udhcpd /var/zebra /var/siproxd /var/cache /var/samba /var/samba/share /var/samba/homes /var/samba/private /var/samba/locks
