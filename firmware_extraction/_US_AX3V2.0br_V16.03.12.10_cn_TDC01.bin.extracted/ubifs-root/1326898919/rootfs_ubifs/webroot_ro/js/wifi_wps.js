...
//更新数据
function updateWps() {
	$.getJSON("goform/WifiWpsGet?" + Math.random(), function(obj) {
		$("#wpsEn").attr("class", (obj.wpsEn == "1" ? "btn-off" : "btn-on"));
		changeWpsEn();
		if (obj.wpsEn == "1") {
			$("#wpsEnbTips").removeClass("none");
		} else {
...
function getWpsStatus() {
	clearTimeout(updateWpsTimer);
	updateWpsTimer = setTimeout(function() {
		updateWps();
	}, WPS_UPDATE_TIME);
}
