...
var netInfo = {
	...
	changeIspType: function () {
        ...
        if (isp_type == "5" && netInfo.initObj.wl_mode !== "wisp") {
            ...
            //wisp下没有pppoe拨号
            $("#netWanType").html(netInfo.initObj.wl_mode == "wisp" ? '<option value="0">' + _("Dynamic IP Address") + '</option><option value="1">' + _("Static IP Address") + '</option>':
...
function getSubData() {
	...
	if (isp_type !== "3") {
		$("#internetVlanId").val(netInfo.initObj.wanInfo[wanIndex - 1].internetVlanId);
		$("#iptvVlanId").val(netInfo.initObj.wanInfo[wanIndex - 1].iptvVlanId);
	}

	netInfo.reboot = false;
	//Unifi/Maxis/Maxis-Special/Manual这四种模式相互切换、Manual修改vlan Id 需要重启设备
	if (CONFIG_SELECT_ISP == 'y') {
		if (((isp_type !== netInfo.initObj.wanInfo[wanIndex - 1].ispType) && (!((isp_type == "0" || isp_type == "5") && (netInfo.initObj.wanInfo[wanIndex - 1].ispType == "0" || netInfo.initObj.wanInfo[wanIndex - 1].ispType == "5")))) ||
			(isp_type == "3" && (internetVlanId !== netInfo.initObj.wanInfo[wanIndex - 1].internetVlanId || iptvVlanId !== netInfo.initObj.wanInfo[wanIndex - 1].iptvVlanId))) {
...
function initWan(obj) {
	...
	if (CONFIG_SELECT_ISP == 'y') {
		//wisp模式下不需要ISPType
		obj.wl_mode == "wisp" ? $("#ispTypeGroup").addClass("none") : $("#ispTypeGroup").removeClass('none');
		$("#ispType").val(obj.wanInfo[wanIndex - 1].ispType||netInfo.ispType);
		netInfo.changeIspType();
		$("#internetVlanId").val(obj.wanInfo[wanIndex - 1].internetVlanId);
		$("#iptvVlanId").val(obj.wanInfo[wanIndex - 1].iptvVlanId);
	} else {
