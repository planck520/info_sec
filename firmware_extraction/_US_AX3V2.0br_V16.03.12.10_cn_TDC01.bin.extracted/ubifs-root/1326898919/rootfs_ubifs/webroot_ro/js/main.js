...
function PageLogic(pageView, pageModel) {
    ...
    this.onMenuClick = function (e) {
        var curId = $(".nav-list.active a")[0].href.split('#')[1],
            targetId = (e.target.href || e.target.parentNode.href).split('#')[1];
...
staInfo = {
    ...
    init: function () {
        ...
        if (!staInfo.loading) {
            ...
            $("#statusWanInternet").on("click", function () {
                //WAN1口
                staInfo.wanTargetIndex = "1";
                showIframe(_("Internet Settings"), "net_set.html", 800, 490);
            });

            //桥接上级失败时
            $("#statusRepeaterWrap").on("click", function () {
    ...
    setImage: function (obj) {
        ...
        $("#repeaterMode").addClass("none");

        //$("#statusDeviceName").text(obj.deviceName);
        $("#statusDeviceName").text("Router");
    ...
    showWorkMode: function (workMode) {
        ...
        if (workMode == "ap" || workMode == "client+ap") {
            ...
            if (workMode == "client+ap") {
                hideArr.push("wrl_wps","wrl_wifi_time","adv_sleepMode");
            }
    ...
    setWispMode: function (obj) {
        ...
        $("#statusRepeaterDisconnected").addClass("none");
        $("#statusWispConnected").addClass("none");
        $("#statusClientConnected").addClass("none");
        ...
        if (wispConnectStatus.charAt(2) == "1") { //连接成功
            ...
            $("#statusWispConnected").removeClass("none");
            $("#statusWispUpSpeed").html(translateSpeed(uploadSpeedStr));
            $("#statusWispDownSpeed").html(translateSpeed(downloadSpeedStr));
        } else {
            //连接失败时不显示实时网速
            $("#statusRepeaterDisconnected").removeClass("none");
        }
    },
    ...
    setApclientMode: function (obj) {
        ...
        if (apConnectStatus == "2003") {
            $("#statusClientConnected").removeClass("none");
        } else {
            $("#statusRepeaterDisconnected").removeClass("none");
        }
    },
    ...
    setWlStatus: function (obj) {
        ...
        if (obj.doubleBand === "1") { //双频合一 无线必定是开启的
            $("#status5GWrap").hide();
            $("#status24GWrap .status-round").html("2.4G/5G:");
            if (obj.wl24gEn == "1") {
                $("#statusSsid").html(toHtmlCode(obj.wl24gName)).attr("title", obj.wl24gName);
                $("#status24GWrap .status-round").addClass("active");
            } else {
                $("#status24GWrap .status-round").removeClass("active");
                $("#statusSsid").html(_("Disable"));
            }
        } else {
            ...
            if (obj.wl5gEn == "1") {
                ...
                $("#statusSsid_5g").html(toHtmlCode(obj.wl5gName)).attr("title", obj.wl5gName);
            } else {
                $("#status5GWrap .status-round").removeClass("active");
                $("#statusSsid_5g").html(_("Disable"));
            }
        ...
        if ((wan_type === "3") || (wan_type === "4")) {
            if (netInfo.ipv6Flag === "1") { //IPv6开启
                if (netInfo.clientFlag[0] === "1") { //VPN开启
                    $(".select-tab").html(_("When connection type is Russia PPTP/Russia L2TP, VPN and IPv6 function will be disabled"));
                } else {
                    $(".select-tab").html(_("When connection type is Russia PPTP/Russia L2TP, IPv6 function will be disabled"));
                }
            } else { //IPv6关闭
                if (netInfo.clientFlag[0] === "1") { //VPN开启
                    $(".select-tab").html(_("When connection type is Russia PPTP/Russia L2TP, VPN function will be disabled"));
                } else {
                    $(".select-tab").html(_(""));
                }
            }
...
netInfo = {
    ...
    wl_mode:"",
    ispType: "0", //默认isp类型为Normal
    // VlanID_Internet : {"1":"500","2":"621","3":"11"},//不同运营商Internet Vlan ID的默认值不同
    // VlanID_Iptv : {"1":"600","2":"823","3":"17"},//不同运营商IPTV Vlan ID的默认值不同
    init: function () {
        //判断是否修改了WAN参数
        netInfo.wanBtnChange = false;
        netInfo.wanBtnChange2 = false;
        if (!netInfo.loading) {
            $("#ispType").on("change",netInfo.changeIspType)
            ...
            $("#internetVlanId").inputCorrect("num");
            $("#iptvVlanId").inputCorrect("num");
        ...
        if (CONFIG_SELECT_ISP == 'y') {
            $("#ispTypeGroup").removeClass('none');
        }
        ...
        $.GetSetData.getJson("goform/getWanParameters?" + Math.random(), function (obj) {
            ...
            netInfo.wl_mode = obj.wl_mode;

            if ( CONFIG_SELECT_ISP == 'y') {
                //避免isp类型与wanType冲突或者isp未传值的情况
                if (obj.wanInfo[0].wanType > 2) {
                    $("#ispType").val("5");
                } else {
                    $("#ispType").val(obj.wanInfo[0].ispType||netInfo.ispType);
                }
                netInfo.changeIspType();
                $("#internetVlanId").val(obj.wanInfo[0].internetVlanId);
                $("#iptvVlanId").val(obj.wanInfo[0].iptvVlanId);
                //wisp模式下不需要ISPType
                if (obj.wl_mode === "wisp") {
                    $("#ispTypeGroup").addClass("none");
                }
            } else {
                //wisp下没有pppoe拨号 和 Russia接入方式
                var wanOptStr = '<option value="0">' + _("Dynamic IP Address") + '</option><option value="1">' + _("Static IP Address") + '</option>';
    ...
    checkWanData: function (formWrap) {
        ...
        var lanIp = netInfo.initObj.lanIp,
            ...
            ispType = $("#ispType").val(),
            internetVlanId = $("#internetVlanId").val(),
            iptvVlanId = $("#iptvVlanId").val(),
            wanObj = {
                wanForm: "wan1SetWrap",
        ...
        for (var i = 0; i < dataArr.length; i++) {
            ...
            if (btn_val == _("Connect")) {
                ...
                if ( CONFIG_SELECT_ISP == 'y' && ispType == "3") {
                    //VLAN ID的范围为[4-4094]
                    if (parseInt(internetVlanId, 10) > 4094 || parseInt(internetVlanId, 10) < 2 || parseInt(iptvVlanId, 10) > 4094 || parseInt(iptvVlanId, 10) < 2 ) {
                        return _("The VLAN ID range is %s.", ["2-4094"]);
                    }
                     //VLAN ID的值不能相同
                    if ( internetVlanId == iptvVlanId) {
                        return _("Internet VLAN ID and IPTV VLAN ID can not be the same");
                    }
                }
    ...
    preSubmit: function () {
        var subData,
            ...
            subObj;
            //若用户未选择手动设置ISP，则清空之前预设的vlanID 保留为初始获取的VLANID值
            if (isp_type !== "3") {
                $("#internetVlanId").val(netInfo.initObj.wanInfo[0].internetVlanId);
                $("#iptvVlanId").val(netInfo.initObj.wanInfo[0].iptvVlanId);
            }
        ...
        //Unifi/Maxis/Maxis-Special/Manual这四种模式相互切换、Manual修改vlan Id 需要重启设备
        if(CONFIG_SELECT_ISP == 'y'){
            if (((isp_type !== netInfo.initObj.wanInfo[0].ispType) && (!((isp_type == "0"||isp_type == "5") &&(netInfo.initObj.wanInfo[0].ispType == "0"||netInfo.initObj.wanInfo[0].ispType =="5")))) ||
            (isp_type == "3" && (internetVlanId !== netInfo.initObj.wanInfo[0].internetVlanId||iptvVlanId !== netInfo.initObj.wanInfo[0].iptvVlanId))) {
    ...
    changeIspType: function () {
        var isp_type = $("#ispType").val();
        if (isp_type == "5" && netInfo.wl_mode !== "wisp") {
            $("#netWanType").html('<option value="3">' + _("Russia PPTP") + '</option><option value="4">' + _("Russia L2TP") + '</option><option value="5">' + _("Russia PPPoE") + '</option>');
            $("#vlanId_set").addClass("none");
        } else {
            //wisp下没有pppoe拨号
            $("#netWanType").html(netInfo.wl_mode == "wisp" ? '<option value="0">' + _("Dynamic IP Address") + '</option><option value="1">' + _("Static IP Address") + '</option>':
            '<option value="2">' + _("PPPoE") + '</option><option value="0">' + _("Dynamic IP Address") + '</option><option value="1">' + _("Static IP Address") + '</option>');
           
            if (isp_type == "3") {
                $("#vlanId_set").removeClass("none");
            } else {
                $("#vlanId_set").addClass("none");
                // $("#internetVlanId").val(netInfo.VlanID_Internet[isp_type]);
                // $("#iptvVlanId").val(netInfo.VlanID_Iptv[isp_type]);
            }
        } 
        netInfo.changeWanType();
    }
...
guestInfo = {
    ...
    checkData: function() {
        var guestSsid = $("#guestSsid").val(),
            guestSsid_5g = $("#guestSsid_5g").val();
        if (guestSsid == guestInfo.initObj.ssid || guestSsid == guestInfo.initObj.ssid_5g || guestSsid_5g == guestInfo.initObj.ssid || guestSsid_5g == guestInfo.initObj.ssid_5g ) {
            return(_("The WiFi names of the main network and guest network cannot be the same"));
        }
    },
    preSubmit: function () {
        var subData,
        ...
        var msg = guestInfo.checkData();

        if (msg) {
            showErrMsg("guest_save_msg",msg)
            return ;
        }
...
ipv6Info = {
    ...
    init: function () {
        ...
        $.validate.valid.ipv6prefix = {
            all: function (str) {
                var items = str.split("::"),
                    ret = $.validate.valid.ipv6Header.all(str, true);
                if (ret) {
                    return _("Please enter a valid prefix");
                }
    ...
    setValue: function (obj) {
        ...
        if(CONFIG_SELECT_ISP == 'y' && obj.ispType == "5" && (obj.wanType == "3" || obj.wanType == "4")){
            $("#conType option[value = 'PPPoE']").addClass("none");
        }
    ...
    checkData: function () {
        var gateWay = $("#gateWay").val(),
            ...
            wanIpv6Addr = ipv6Info.data.wanAddr.split("/")[0];
            wanipcheck = ipv6Info.data.wanipcheck;
        ...
        if (wan_type == "Static") {
            ...
            if (!prefixDelegate && ipv6Info.checkIpv6InSameSegment(wanipcheck, lanPrefix, 64, 64)) {
                showErrMsg("msg-err-lan", _("IPv6 WAN address and IPv6 LAN address cannot be in the same segment"));
                return false;
            }
         }
    ...
    changeIPv6En: function () {
        ...
        if (ipv6Info.data.wanType == "3" || ipv6Info.data.wanType == "4" || ipv6Info.data.wanType == "5") {
            showErrMsg("msg-err", _("The IPv6 function is not available when WAN connection type is Russia PPPoE/Russia L2TP/Russia PPTP."), false, 7000);
            return;
        }
...
vpnInfo = {
    ...
    setValue: function (obj) {
        ...
        $("#vpn_client").removeClass("disabled");

        if (obj.wanType == "3" || vpnInfo.data.wanType == "4") {
            $("#vpn_client").addClass("disabled");
        }
        ...
        switch (id) {
            ...
            case "vpn_client":
                if (vpnInfo.data.wanType == "3" || vpnInfo.data.wanType == "4") {
                    alert(_("VPN service is not available when connection type is Russia PPTP/Russia L2TP"));
                    return false;
                }
...
advInfo = {
    ...
    setValue: function (obj) {
        ...
        if (CONFIG_SELECT_ISP == 'y') {
            //如果ispType为Manual、Maxis/Maxis-Special，IPTV菜单页面置灰，不允许用户点击进入
            if(obj.ispType == "2" || obj.ispType == "3" || obj.ispType == "4" ){
                $("#advance #adv_iptv").addClass("disabled");
            }
        }
        advInfo.data = obj;
