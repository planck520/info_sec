...
var pageModel = R.pageModel({
	...
	beforeSubmit: function () {
		...
		if ( CONFIG_SELECT_ISP == "y") {
			var iptvTy_tde = $("#iptvType_tde input[type='text']").val();
			iptvTy = isNaN(iptvTy_tde) ? "none": iptvTy_tde;
		} else {
			iptvTy = $("#iptvType").val();
		}
		...
		if (iptvTy == "none") {
			...
			//TDE版本的自定义vlanID值
			if (parseInt(iptvTy, 10) > 4094 || parseInt(iptvTy, 10) < 4) {
...
function initIptvEvent() {
	...
	//海外版自定义VLAN为输入框形式，且无上海选项
	if ( CONFIG_SELECT_ISP == "y") {
		$("#iptvType_tde").removeClass("none");
		var selectObj = {
			"initVal": _("Default"),
			"editable": "1",
			"seeAsTrans": true,
		...
		$("#iptvType_tde").toSelect(selectObj);
		$("#iptvType_tde input[type='text']").inputCorrect("num").on("blur", function () {
			$("#iptvType_tde input:nth-child(2)").val($("#iptvType_tde input[type='text']").val());
			$.validate().check($("#iptvType_tde"));
        });
	} else {
		$("#iptvType").removeClass("none");
	}
	...
	$("#iptvType_tde").on("click", changeTypeTde);
...
function changeTypeTde() {
	isNaN($("#iptvType_tde input:first").val()) && $("#iptvType_tde input:first").val()!=="" ?
	 $("#iptvSpan").removeClass("none"): $("#iptvSpan").addClass("none");
	$("#iptvType_tde input:nth-child(2)").val($("#iptvType_tde input[type='text']").val());
}
...
function changeSTBEn() {
	...
	if (className == "btn-off") {
		...
		$("#stbEn").val(1);
		if ( CONFIG_SELECT_ISP == "y") {
			G.initObj.ispType!=="1" && $("#vlan_set").removeClass("none");
		} else {
...
function initValue(obj) {
	...
	if (CONFIG_SELECT_ISP == "y") {
		//ISP Type为Unifi时，IPTV页面仅显示 STB开关
		if (obj.ispType == "1") {
			$("#iptv_set .control-group").first().addClass("none");
			$("#vlan_set").addClass("none");
		}

		if(obj.iptvType =="none"){
			$("#iptvType_tde .input-box").val(_("Default"));
		}else{
			$("#iptvType_tde .input-box").val(obj.vlanId);
		}
		changeTypeTde();
		$("#vlanList").addClass("none");
		$("#area_set").addClass("none");
