...
//开启FC开关
function setFCEn() {
	var iframeSrc = top.$(".main-dailog").find("iframe").attr("src");
	//在线列表和网速控制关闭后需要主动开启FC
	var isPageClose = iframeSrc.indexOf("net_control") != -1 || iframeSrc.indexOf("online_list") != -1;

	if(isPageClose) {
		$.GetSetData.setData("goform/setFCCfg", {"action": 1});
	}
}
...
function showSaveMsg (num, str, flag, change) {
	...
	if (num == 0) {
		...
		$(".save-msg").removeClass("none");
		$("#page-message").html(_("Uploading file"));
...
R.extend(R, {
	...
	isTimeOut: function (str) {
		if (str.indexOf("<!DOCTYPE html") === 0 && str.indexOf("login.js?4cdfafd6392d58c98648733c1d18de89") != -1) {
			return true;
		}
...
function translateDeviceType (str) {
	...
	if (top.G.deviceList.indexOf(str) != -1) {
		...
		//设备未支持的手机图标
		obj = {
			src: "./img/device/others.png?4cdfafd6392d58c98648733c1d18de89",
			exsit: false,
