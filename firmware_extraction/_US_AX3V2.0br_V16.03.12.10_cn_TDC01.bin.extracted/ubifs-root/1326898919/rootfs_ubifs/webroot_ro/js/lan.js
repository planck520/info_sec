...
function checkData() {
	G.validate = $.validate({
		custom: function () {
			...
			if (top.CONFIG_SELECT_ISP == "y" && iptvWanIp && checkIpInSameSegment(iptvWanIp, iptvWanMask, lanIp, lanMask)) {
				return _("%s and %s (%s) must not be in the same network segment.", [_("LAN IP Address"), _("WAN IP Address"), iptvWanIp]);
			}
