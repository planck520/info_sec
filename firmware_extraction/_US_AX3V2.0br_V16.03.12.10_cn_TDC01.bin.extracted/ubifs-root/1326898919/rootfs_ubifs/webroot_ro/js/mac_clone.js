...
function updateSpeed(data) {
	for (let i = 0; i < data.wanInfo.length; i++) {
		var dom = i === 0 ? "#currentSpeed" : "#currentSpeed2";
		if (parseInt(data.wanInfo[i].currentSpeed) >= 1 && parseInt(data.wanInfo[i].currentSpeed) <= 6) {
			$(dom).html(currentSpeedInfo[data.wanInfo[i].currentSpeed]);
		} else {
			$(dom).html("");
		}
	}
...
function initValue(obj) {
	speedTimer = setInterval(function () {
		$.getJSON("goform/AdvGetMacMtuWan?" + Math.random(), updateSpeed);
	}, 5000);
	top.$(".main-dailog").removeClass("none");
