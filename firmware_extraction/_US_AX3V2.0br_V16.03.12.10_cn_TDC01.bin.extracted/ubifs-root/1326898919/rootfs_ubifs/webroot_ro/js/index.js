...
// var VlanID_Internet = {"1":"500","2":"621","3":"11"},//不同运营商Internet Vlan ID的默认值不同
...
$(function () {
    ...
    G.validate = $.validate({
        ...
        success: function () {
            ...
            if(ispType !== "3"){
                internetVlanId = wanData.internetVlanId;
                iptvVlanId = wanData.iptvVlanId;
            }
            ...
            var internetObj = [{
                ...
                "cloneType": cloneType,
                "mac": mac,
                "internetVlanId":internetVlanId,
                "iptvVlanId":iptvVlanId
            }, {
                "ispType":ispType,
                "netWanType": netWanType,
                "staticIp": staticIp,
...
function getInitData () {
    ...
    $.getJSON("goform/getWanParameters?" + Math.random(), function (res) {
        wanData = "";
        wanData = res && res.wanInfo && res.wanInfo[0];
        window.quickInit = false;
         if (CONFIG_SELECT_ISP == 'y') {
            // $("#ispType").val(wanData.ispType);
            // ispTypeSelect(wanData.ispType);
            $("#internetVlanId").val(wanData.internetVlanId);
            $("#iptvVlanId").val(wanData.iptvVlanId);
        }
        if (wanData) {
            window.quickInit = true;
            $("#netWanType").val(wanData.wanType);
            wanTypeSelect(wanData.wanType)
            // pppoe
...
function wanTypeSelect (wan_type) {
    ...
    window.quickInit || $('#netType-form input:text, #netType-form input:password').val("");
...
function ispTypeSelect (isp_type) {
    window.quickInit || $('#internet-form input:text, #internet-form input:password').val("");
    switch (parseInt(isp_type)) {
        case 3:
            $("#netWanType").html('<option value="2">' + _("PPPoE") + '</option><option value="0">' + _("Dynamic IP Address") + '</option><option value="1">' + _("Static IP Address") + '</option>');
        ...
        case 5:
            ...
            break;
        default:
            // $("#internetVlanId").val(VlanID_Internet[isp_type]),
            // $("#iptvVlanId").val(VlanID_Iptv[isp_type]),
            $("#netWanType").html('<option value="2">' + _("PPPoE") + '</option><option value="0">' + _("Dynamic IP Address") + '</option><option value="1">' + _("Static IP Address") + '</option>');
            $("#vlanId_set").addClass("none");
            break;
    }
...
function initEvent () {
    ...
    $("#ispTypeGroup").on("change", function () {
        ispTypeSelect($("#ispType").val())
    });
...
function initValue (obj) {
    ...
    if (G.data.line == 1) { //已插网线
        ...
        if (G.data.net == 1) { //是否检测完
            ...
            if (CONFIG_SELECT_ISP == 'y')
               ...
               //俄罗斯和乌克兰
                if (G.countryCode === "RU" || G.countryCode === "UA") {
                    ...
                    $("#netWanType").append('<option value="3">' + _("Russia PPTP") + '</option><option value="4">' + _("Russia L2TP") + '</option><option value="5">' + _("Russia PPPoE") + '</option>');
                }
            }
            //初始化推荐的联网方式
            if (G.data.wanType === 0 || G.data.wanType === 1 || G.data.wanType === 2) {
              recommendType = G.data.wanType;
            } else if (CONFIG_SELECT_ISP == "y" && (G.data.outType > 2 || G.data.outType < 0)) {
              recommendType = 2;
            } else {
            ...
            //         ispType = "5";
            //     } else {
            //         ispType = G.data.ispType;     
            //     }
            //     $('#ispType').val(ispType);
            //     ispTypeSelect(ispType);
            // }
    ...
    /*else if (!$("#dhcp_setting").hasClass("none")) {
        ...
        if ((wrlPwd !== "") && ($("#hideWrlPwd")[0].checked === false)) {
            ...
            var subObj = {
                ...
                // "wifi6En": $("#wifi6En").prop("checked")? 1:0
            };
...
function handWifi (data) {
    ...
    if ( CONFIG_SELECT_ISP == 'y') {
        var ispType = $("#ispType").val();
        //选择Unifi/Maxis/Maxis-Special/Manual这四种模式会重启
        if ((ispType !=="0" && ispType !=="5")) {
            if (confirm(_("Your settings will take effect after the system reboots. Do you want to reboot the system?"))) {
                ...
                html = '<div id="loading_div" >' +
                    '<div class="load-img"><span id="load_pc" class="up-loadding load-reboot"></span></div><br />' + _("Rebooting... Please wait.") + '<span id="load_text"></span>' +
                    '</div>';
                $(html).appendTo("body");
