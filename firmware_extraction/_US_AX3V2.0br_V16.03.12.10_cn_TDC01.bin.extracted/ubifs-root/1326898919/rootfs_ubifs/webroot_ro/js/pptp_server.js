	...
	G.validate = $.validate({
		custom: function () {
			var i = 0,
				...
				startIp,
				endIp,
				iptvWanIp = G.data.iptvWanIp,
				iptvWanMask = G.data.iptvWanMask,
				msg = "";
			...
			if ($("#serverEn").val() == "1") {
				...
				if (top.CONFIG_SELECT_ISP == "y" && iptvWanIp && checkIpInSameSegment(startIp, pptpSvrMask, iptvWanIp, iptvWanMask)) {
					return _("The start IP address of IP Address Pool must not be in the same network segment as %s (%s).", [_("WAN IP Address"), iptvWanIp]);
				}
				// 决策： 访客网络网段冲突有后台处理
