		...
		$.getJSON("goform/getLoginInfo" + "?" + Math.random(), function (data) {
			if (data.isLimit == "1") { //超出限制
				...
				if (data.isLocked === '1') {
					...
					if (data.leftTimes == '1')
						$('#login-message').html(_("Incorrect password, please try again. 1 attempt left"));
					else
						$('#login-message').html(_("Incorrect password, please try again. %s attempts left", [data.leftTimes]));
